...
class MyMuseHelperAmazons3 extends JObject
{
	...
	*/
	public static function putObject($input, $bucket, $uri, $acl = null, $metaHeaders = array(), $requestHeaders = array()) {
		...
		if(empty($bucket)) $bucket = self::$__default_bucket;
		$rest = new MyMuseS3Request('PUT', $bucket, $uri);
	...
	*/
	public static function startMultipart($input, $bucket, $uri, $acl = null, $metaHeaders = array(), $requestHeaders = array()) {
		if ($input === false) return false;
		if(empty($bucket)) $bucket = self::$__default_bucket;
		$rest = new MyMuseS3Request('POST', $bucket, $uri);
	...
	*/
	public static function uploadMultipart($input, $bucket, $uri, $acl = null, $metaHeaders = array(), $requestHeaders = array()) {
		...
		$rest = new MyMuseS3Request('PUT', $bucket, $uri, 's3.amazonaws.com');
		...
		if (isset($input['file'])) {
			...
			$rest->fp = $fp; // I have to set the MyMuseS3Request's file pointer, NOT the file structure's, as the request object is already initialized
			$rest->data = false;
		} elseif (isset($input['data'])) {
			$tempfilename = null;
			$rest->fp = false;
	...
	*/
	public static function getObject($bucket, $uri, $saveTo = false, $from = null, $to = null) {
		if(empty($bucket)) $bucket = self::$__default_bucket;
		$rest = new MyMuseS3Request('GET', $bucket, $uri);
		if ($saveTo !== false) {
	...
	*/
	public static function deleteObject($bucket, $uri) {
		if(empty($bucket)) $bucket = self::$__default_bucket;
		$rest = new MyMuseS3Request('DELETE', $bucket, $uri);
	...
	*/
	public static function getBucket($bucket, $prefix = null, $marker = null, $maxKeys = null, $delimiter = null, $returnCommonPrefixes = false) {
		if(empty($bucket)) $bucket = self::$__default_bucket;
		$rest = new MyMuseS3Request('GET', $bucket, '');
	...
	*
	* @internal Used by MyMuseS3Request::getResponse()
	...
	*/
	...
	/**
	 * Method to get the file lists in amazons3
	 *
	 * @access    public
	 ...
	 * @return    array
	 */
	public function listS3Contents($directory = null, $bucket = '')
	{
		$params = MyMuseHelper::getParams();
...
final class MyMuseS3Request {
	...
	*/
	public function getResponse() {
		...
		$url = ((MyMuseHelperAmazons3::$useSSL && extension_loaded('openssl')) ?
		'https://':'http://').$this->headers['Host'].$this->uri;

		// Basic setup
		$curl = curl_init();
		@curl_setopt($curl, CURLOPT_CAINFO, MYMUSE_CACERT_PEM);
		...
		$headers[] = 'Authorization: ' . MyMuseHelperAmazons3::__getSignature($stringToSign);
