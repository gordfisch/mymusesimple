	...
	<fieldset id="filter-bar">
		...
		<div class="filter-select fltrt">
			<select name="filter_featured" class="inputbox" onchange="this.form.submit()">
				<option value=""><?php echo JText::_('MYMUSE_SELECT_FEATURED');?></option>
				<?php echo JHtml::_('select.options', $this->featured, 'value', 'text', $this->state->get('filter.featured'), true);?>
			</select>
			...
			<select name="filter_category_id" class="inputbox" onchange="this.form.submit()">
				...
				<?php echo JHtml::_('select.options', JHtml::_('category.options', 'com_mymuse'), 'value', 'text', $this->state->get('filter.category_id'));?>
			</select>
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td class="order">
					<?php if ($canChange) : ?>
						...
						if ($saveOrder) :?>
							<?php if ($listDirn == 'asc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, ($item->catid == @$this->items[$i-1]->catid), 'products.orderup', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, ($item->catid == @$this->items[$i+1]->catid), 'products.orderdown', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php elseif ($listDirn == 'desc') : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, ($item->catid == @$this->items[$i-1]->catid), 'products.orderdown', 'JLIB_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, ($item->catid == @$this->items[$i+1]->catid), 'products.orderup', 'JLIB_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php endif; ?>
						<?php endif; ?>
