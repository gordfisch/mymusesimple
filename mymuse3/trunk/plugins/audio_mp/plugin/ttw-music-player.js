...
(function($) {
    $.fn.ttwMusicPlayer = function(playlist, userOptions) {
        var $self = this, defaultOptions, options, cssSelector, appMgr, playlistMgr, interfaceMgr, ratingsMgr, playlist,
                layout, ratings, price, myPlaylist, current;
        ...
        defaultOptions = {
            ratingCallback:function(index, playlistItem, rating){
            	...
            	var encodedArtist = encodeURIComponent(playlistItem.artist);
            	var encodedTitle = encodeURIComponent(playlistItem.title);
            	var url = 'index.php?option=com_mymuse&task=rateajax&index='+index+'&cat_prod='+encodedArtist+'&rating='+rating+'&title='+encodedTitle;
            	var result = false;
                ...
                $.ajax({
                  ...
                  success: function(data) {
                       result = true;
                       document.getElementById('ratings_return').innerHTML = data;
                  }
                });
                if (result == false) { document.getElementById('ratings_return').innerHTML = '<p>Trying to refresh</p>'; }
        ...
        playlistMgr = function() {
            ...
            markup = {
                listItem:'<li class="track">' +
                            ...
                            '<input id="" class="buy  not-active" type="checkbox" name="productid[]" value="" />' +
                        '</li>',
            ...
            function setBuyLink($track, index) {
                if (!isUndefined(myPlaylist[index].buy)) {
                	boxnum = index+1+window.precount;
                    $track.find(cssSelector.buy).removeClass(attr(cssSelector.buyNotActive)).attr('value', myPlaylist[index].buy).attr('id', 'box'+boxnum);
                    $track.find(cssSelector.price).html(buyText(index));
                }
            }
        ...
        interfaceMgr = function() {
            ...
            function buildInterface() {
                ...
                markup = '<div class="ttw-music-player">' +
                        ...
                        '                </div>' +
                        '            </div><div id="ratings_return" class="artist"></div>' +
                        '        </div>' +
                        '    </div>' +
                        ...
                        '</div>';
                var product_player = document.getElementById('product_player');
                $interface = $(markup).css({display:'none', opacity:0}).appendTo(product_player).slideDown('slow', function() {
