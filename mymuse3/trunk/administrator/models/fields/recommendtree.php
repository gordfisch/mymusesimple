 ...
 * @since		1.6
 */
class JFormFieldRecommendTree extends JFormFieldList{
	/**
	 * The form field type.
	 *
	 * @var		string
	...
	protected function getInput() {
		...
		if($id){
			...
			foreach($cats as $cat){
				$this->value[$cat->recommend_id] = $cat->recommend_id;
			}
		}
		
		//a catid coming in
		$catid = JRequest::getVar('recommend', 0);
		if($catid){
	...
	function _getCatAncestors( $id, $indent, $list, &$children, $title, $maxlevel=9999, $level=0, $type=1, $ancestors=null )
	{
		...
		if (@$children[$id] && $level <= $maxlevel) {
			foreach ($children[$id] as $v) {
				...
				$list = JFormFieldRecommendTree::_getCatAncestors( $id, $indent.$spacer, $list, $children, $title, $maxlevel, $level+1, $type, $ancestors );
			}
		}
		return $list;
	}
    ...
    */
	function _getDescendants($arr, &$descendants = array())
	{
		foreach($arr as $k => $v)
		{
			...
			if ($v->childrenarray) {
				JFormFieldRecommendTree::_getDescendants($v->childrenarray, $descendants);
			}
		}
		return $descendants;
	}
