...
class plgMyMusePayment_Paypalpro extends JPlugin
{
	...
	function onMyMuseNotify($params, $Itemid = 1)
	{
		...
		if(isset($data['mode']) && $data['mode'] == 'init') {
			...
			//$path = JURI::root(true);
			if(!$isValid ){
				$thankyouUrl = 'index.php?option=com_mymuse&task=paycancel&view=cart&pp=paypalpaymentspro&orderid='.$orderid.'&Itemid='.$Itemid;
				$thankyouUrl = JURI::root().$thankyouUrl;
				$msg = "Payment Failed: ".$result ['error'];
			}else{
				$thankyouUrl = 'index.php?option=com_mymuse&task=thankyou&view=cart&pp=paypalpaymentspro&orderid='.$orderid.'&Itemid='.$Itemid;
				$thankyouUrl = JURI::root().$thankyouUrl;
				$msg = "";
			}
