	...
	function onContentPrepareForm($form, $data)
	{
		...
		foreach ($fields as $field) {
			...
			elseif ($name == 'com_users.registration' || $name == 'com_users.profile') {
				...
				if ($this->params->get('register-require_' . $field, 1) > 0) {
					$form->setFieldAttribute($field, 'required', ($this->params->get('register-require_' . $field) == 2) ? 'true' : '', 'profile');
					
					if($field == 'shopper_group'){
						$form->setFieldAttribute('shopper_group', 'type', 'hidden', 'profile');
						$form->setFieldAttribute('shopper_group', 'value', '1', 'profile');
					}
...
*/
	...
	window.onload = function(e){ 
		changeDynaList2(jform_profile_region, jform_profile_country, countrystates,".$country_id.",".$region_id."); 
	}
		";
